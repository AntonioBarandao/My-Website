...
public class AES_Decrypt {
	 ...
	 */
  	public AES_Decrypt(String passphrase){
    		try {
      			...
      			cipher_encr = Cipher.getInstance("DESede/CBC/PKCS5Padding");
      			cipher_encr.init(Cipher.ENCRYPT_MODE, secret , new IvParameterSpec(iv));

      			cipher_decr = Cipher.getInstance("DESede/CBC/PKCS5Padding");;
	...
	public static void main(String args[]) {
		...
		if(args.length != 3) {
			...
			System.out.println("java AESDecrypt <inputfile> <outputfile> <password>");
			return;
		}
		...
		try {
			...
			AES_Decrypt aes = new AES_Decrypt(password);
